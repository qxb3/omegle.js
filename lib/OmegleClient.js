...
class Omegle extends EventEmitter {
  constructor(options) {
    ...
    this.baseURL = 'https://front10.omegle.com'
  ...
  connect(options) {
    ...
    axios.post(`${this.baseURL}/start?caps=recaptcha2,t2&firstevents=1&lang=${options?.lang || this.defaultLang}&topics=[${options?.topics?.map(v => `"${v}"`).join(',') || ''}]`)
      .then(({ data }) => {
